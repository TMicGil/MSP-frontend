    ...
    const searchCity = (query) => {
      ...
      .then(({ features }) => {
        const suggestions = features.map((data, i) => {
          return { id: i, title: data.properties.label, context: data.properties.context, latitude: data.geometry.coordinates[1], longitude: data.geometry.coordinates[0] };
        });
...
// FUNCTION POUR TOUT ENVOYER EN DATABASE -- CREATION NEW EVENT
    const handleConfirm = () => {
      ...
      .then((data) => {
        if (data.result) {
          Alert.alert("Confirmation :", "Your event has been created !", {cancelable: true})
        } else {
